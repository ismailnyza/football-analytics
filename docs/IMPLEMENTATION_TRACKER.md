...
- SQLite publish action and browser | TESTED | Added migration `0003_published_entities.sql`, a SQLite-backed published-entity store, and `PublishSourceRecords`. Data / Import now publishes the selected source on `p`, marks staged rows processed, and renders persisted published entities from the SQLite publish store alongside raw/normalized/publish-candidate previews. |
