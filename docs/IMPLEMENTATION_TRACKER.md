...
- Fetch metadata is persisted in `ingest_state.json` (per-source caps default 2500/run, editable in the file). `simcli scrape` performs real HTTP to user-supplied FBref URLs; obey site terms, robots.txt, and rate limits. Raw staged payloads now persist in SQLite, but there is still no publish/browser workflow for those rows inside the TUI.
