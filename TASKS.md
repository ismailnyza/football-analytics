...
- broaden published-entity workflows beyond review/edit controls into explicit approval or branch-import actions
...
- Data / Import can now publish the selected source into a SQLite `published_entities` store with `p`, mark staged rows processed, browse persisted published entities in-screen, inspect entity detail, and edit persisted `name` / `resolved_id` values from the TUI.
